  ...
  <div class="bg-radius">
    ...
    <form class="form-bottom">
      <div class="form-group row">
        ...
        <div class="col-sm-12">
          <input type="text" class="form-control insure-input" id="txtOldAddress1" :value="GetHealthPostData.address1" disabled="disabled">
        </div>
...
export default {
  ...
  mounted() {
    this.tempZip1 = this.GetHealthPostData.zip1
    this.tempCity1 = this.GetHealthPostData.city1
    this.tempDistrict1 = this.GetHealthPostData.district1
    this.tempRoad1 = this.GetHealthPostData.road1
  },
  computed: {
    ...mapGetters([
      'GetCityData',
    ...
    city1: {
      get() {
        return this.GetHealthPostData.city1 || '0'
      },
      set(value) {
        this.GetHealthPostData.city1 = value
        // 重新選取縣市, 要更新區域下拉框並清空區域原先的值
        this.FuncGetDistrictData(value)
        this.GetHealthPostData.district1 = 0
      }
    },
    // 輸入新的通訊地址-區域
    district1: {
      get() {
        let result = InitColumnData(this.GetHealthPostData.district1, '0')
        if (result === '0') return '0'
        return (`${this.GetHealthPostData.zip1}-${this.GetHealthPostData.district1}`) || 0
      },
      set(value) {
        // item.Zip|item.Area
        let data = value.split('-')
        this.GetHealthPostData.zip1 = data[0]
        this.GetHealthPostData.district1 = data[1]
      }
    },
    // 輸入新的通訊地址-路
    road1: {
      get() {
        return this.GetHealthPostData.road1
      },
      set(value) {
        this.GetHealthPostData.road1 = value
      }
    }
  },
  methods: {
    ...
    OnCommunityAddr(target) {
      switch (target) {
        case 'old':
          ...
          this.GetHealthPostData.zip1 = this.tempZip1
          this.GetHealthPostData.city1 = this.tempCity1
          this.GetHealthPostData.district1 = this.tempDistrict1
          this.GetHealthPostData.road1 = this.tempRoad1
          break
        case 'new':
