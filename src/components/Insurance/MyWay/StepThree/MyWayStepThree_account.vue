...
export default {
  computed: {
    ...
    Applicant_BankCode: {
      get() {
        return this.GetMyWayPostData.Applicant_BankCode
      },
      set(value) {
        let data = value.split('-')
        this.FuncGetBankBranches(data[0])
        this.GetMyWayPostData.Applicant_BankCode = data[0]
        this.GetMyWayPostData.Applicant_BankName = data[1]
      }
    },
    // 金融機構分行代號
    Applicant_BranchCode: {
      get() {
        return (this.GetMyWayPostData.Applicant_BranchCode === '' || this.GetMyWayPostData.Applicant_BranchCode === null || this.GetMyWayPostData.Applicant_BranchCode === undefined) ? 0 : this.GetMyWayPostData.Applicant_BranchCode
      },
      set(value) {
        let data = value.split('-')
        this.GetMyWayPostData.Applicant_BranchCode = data[0]
        this.GetMyWayPostData.Applicant_BranchName = data[1]
      }
    },
    // 金融機構中文名稱
    Applicant_BranchName: {
      get() {
        return this.GetMyWayPostData.Applicant_BranchName
      }
    },
    // 銀行帳號
    Applicant_Account: {
      get() {
        return this.GetMyWayPostData.Applicant_Account
      },
      set(value) {
        this.GetMyWayPostData.Applicant_Account = value
      }
    }
  },
  methods: {
