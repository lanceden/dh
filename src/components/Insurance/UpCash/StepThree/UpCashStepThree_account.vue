...
export default {
  computed: {
    ...
    Applicant_BankCode: {
      get() {
        return this.GetUpCashPostData.Applicant_BankCode
      },
      set(value) {
        let data = value.split('-')
        this.FuncGetBankBranches(data[0])
        this.GetUpCashPostData.Applicant_BankCode = data[0]
        this.GetUpCashPostData.Applicant_BankName = data[1]
      }
    },
    // 金融機構分行代號
    Applicant_BranchCode: {
      get() {
        return (this.GetUpCashPostData.Applicant_BranchCode === '' || this.GetUpCashPostData.Applicant_BranchCode === null || this.GetUpCashPostData.Applicant_BranchCode === undefined) ? 0 : this.GetUpCashPostData.Applicant_BranchCode
      },
      set(value) {
        let data = value.split('-')
        this.GetUpCashPostData.Applicant_BranchCode = data[0]
        this.GetUpCashPostData.Applicant_BranchName = data[1]
      }
    },
    // 金融機構中文名稱
    Applicant_BranchName: {
      get() {
        return this.GetUpCashPostData.Applicant_BranchName
      }
    },
    // 銀行帳號
    Applicant_Account: {
      get() {
        return this.GetUpCashPostData.Applicant_Account
      },
      set(value) {
        this.GetUpCashPostData.Applicant_Account = value
      }
    }
  },
  methods: {
