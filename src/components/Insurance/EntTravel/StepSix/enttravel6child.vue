...
export default {
  ...
  computed: {
    ...
    city3: {
      get() {
        return this.GetEntTravelPostData.PolicyData.InsuredInfo[this.index].BeneficiaryData[0].Address.City || 0
      },
      set(value) {
        this.GetEntTravelPostData.PolicyData.InsuredInfo[this.index].BeneficiaryData[0].Address.City = value
        // 重新選取縣市, 要更新區域下拉框並清空區域原先的值
        this.FuncGetDistrictData(value)
        this.GetEntTravelPostData.PolicyData.InsuredInfo[this.index].BeneficiaryData[0].Address.District = 0
      }
    },
    // 身故受益人地址-區域
    district3: {
      get() {
        if (this.GetEntTravelPostData.PolicyData.InsuredInfo[this.index].BeneficiaryData[0].Address.District === 0) return 0
        return (`${this.GetEntTravelPostData.PolicyData.InsuredInfo[this.index].BeneficiaryData[0].Address.District}`) || 0
      },
      set(value) {
        // item.Zip|item.Area
        this.GetEntTravelPostData.PolicyData.InsuredInfo[this.index].BeneficiaryData[0].Address.District = value
      }
    },
    // 身故受益人地址-路
    road3: {
      get() {
        return this.GetEntTravelPostData.PolicyData.InsuredInfo[this.index].BeneficiaryData[0].Address.Street
      },
      set(value) {
        this.GetEntTravelPostData.PolicyData.InsuredInfo[this.index].BeneficiaryData[0].Address.Street = value
      }
    }
  },
  methods: {
