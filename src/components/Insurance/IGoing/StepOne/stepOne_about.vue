  ...
  <div class="bg-radius">
    ...
    <form class="form-bottom">
      ...
      <div class="form-group row">
        ...
        <div class="col-sm-12 insure-select-align">
          <select class="form-control data-input insure-select insure-input-block-edit" v-model="AuthorizedRep_computed">
            <option value="0">請選擇</option>
            <option value="true">是</option>
            <option value="false">否</option>
          </select>
        </div>
        <label class="col-sm-12 col-form-label insure-label" v-show="AuthorizedRepError === true">親愛的客戶謝謝您的申購保險，因相關法規規定您的申請文件需另檢附相關證明文件。很抱歉您無法於本網站進行投保動作。煩請另洽新光人壽服務人員詢問相關保險商品購買事宜，造成您的不便我們深感抱歉，再次感謝您的惠顧。</label>
      </div>
  ...
  computed: {
    ...
    QusAns1: {
      ...
      set(value) {
        ...
        this.GetIGoingPostData.QusAns[0].Answar = !(value === 'true')
      }
    },
    // 請問您過去二年內是否曾因患有下列疾病而接受醫師治療、診療或用藥? (有任一者，請選擇”是”)
    QusAns2: {
      ...
      set(value) {
        ...
        this.GetIGoingPostData.QusAns[2].Answar = !(value === 'true')
      }
    },
    // 請問您目前身體機能是否有下列障害：(有任一者，請選擇”是”)
    QusAns3: {
      ...
      set(value) {
        ...
        this.GetIGoingPostData.QusAns[3].Answar = !(value === 'true')
      }
    }
  },
  methods: {
    test() {
      console.log('this.GetIGoingPostData.AuthorizedRep', this.GetIGoingPostData.AuthorizedRep)
