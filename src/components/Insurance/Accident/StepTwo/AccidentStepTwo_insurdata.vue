...
export default {
  ...
  computed: {
    ...
    po_issue_date: {
      get() {
        return this.GetAccidentPostData.po_issue_date || moment().add(`1`, 'days').format(`YYYY-MM-DD`)
      },
      set(value) {
        this.GetAccidentPostData.po_issue_date = value
        this.GetAccidentPostData.mode_prem = 0
        let maturedDate = moment(this.GetAccidentPostData.po_issue_date, 'YYYY-MM-DD').format(`民國${parseInt(new Date().getFullYear()) + 1 - 1911}年 MM 月 DD 日午夜十二時`)
        $('#matured_date').html(`至 ${maturedDate}`)
      }
    },
    /**
    ...
    face_amt: {
      get() {
        return this.GetAccidentPostData.face_amt || 450
      },
      set(value) {
