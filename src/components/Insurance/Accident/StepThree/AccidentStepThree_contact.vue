...
export default {
  ...
  mounted() {
    // 暫存舊的通訊地址
    this.tempZip2 = this.GetAccidentPostData.zip2
    this.tempCity2 = this.GetAccidentPostData.city2
    this.tempDistrict2 = this.GetAccidentPostData.district2
    this.tempRoad2 = this.GetAccidentPostData.road2
  },
  computed: {
    ...mapGetters([
      'GetCityData',
    ...
    phone_mobile: {
      get() {
        return this.GetAccidentPostData.phone_mobile || ''
      },
      set(value) {
        this.this.GetAccidentPostData.phone_mobile = value
      }
    },
    // 電話區碼
    phone_area: {
      get() {
        return this.GetAccidentPostData.phone_area || ''
      },
      set(value) {
        this.GetAccidentPostData.phone_area = value
      }
    },
    // 電話
    phone_main: {
      get() {
        return this.GetAccidentPostData.phone_main || ''
      },
      set(value) {
        this.GetAccidentPostData.phone_main = value
      }
    },
    // 輸入新的戶籍地址-縣市
    city2: {
      get() {
        return this.GetAccidentPostData.city2 || '0'
      },
      set(value) {
        this.GetAccidentPostData.city2 = value
        // 重新選取縣市, 要更新區域下拉框並清空區域原先的值
        this.FuncGetDistrictData(value)
        this.GetAccidentPostData.district2 = 0
      }
    },
    // 輸入新的戶籍地址-區域
    district2: {
      get() {
        let result = InitColumnData(this.GetAccidentPostData.district2, '0')
        if (result === '0') return '0'
        return (`${this.GetAccidentPostData.zip2}-${this.GetAccidentPostData.district2}`) || 0
      },
      set(value) {
        // item.Zip|item.Area
        let data = value.split('-')
        this.GetAccidentPostData.zip2 = data[0]
        this.GetAccidentPostData.district2 = data[1]
      }
    },
    // 輸入新的戶籍地址-路
    road2: {
      get() {
        return this.GetAccidentPostData.road2
      },
      set(value) {
        this.GetAccidentPostData.road2 = value
      }
    }
  },
  methods: {
    ...
    OnRegisterAddr(target) {
      switch (target) {
        case 'old':
          ...
          this.GetAccidentPostData.zip2 = this.tempZip2
          this.GetAccidentPostData.city2 = this.tempCity2
          this.GetAccidentPostData.district2 = this.tempDistrict2
          this.GetAccidentPostData.road2 = this.tempRoad2
          break
        case 'new':
