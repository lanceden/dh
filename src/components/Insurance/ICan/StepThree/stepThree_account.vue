...
export default {
  computed: {
    ...
    Applicant_BankCode: {
      get() {
        return this.GetICanPostData.Applicant_BankCode
      },
      set(value) {
        let data = value.split('-')
        this.FuncGetBankBranches(data[0])
        this.GetICanPostData.Applicant_BankCode = data[0]
        this.GetICanPostData.Applicant_BankName = data[1]
      }
    },
    // 金融機構分行代號
    Applicant_BranchCode: {
      get() {
        return (this.GetICanPostData.Applicant_BranchCode === '' || this.GetICanPostData.Applicant_BranchCode === null || this.GetICanPostData.Applicant_BranchCode === undefined) ? 0 : this.GetICanPostData.Applicant_BranchCode
      },
      set(value) {
        let data = value.split('-')
        this.GetICanPostData.Applicant_BranchCode = data[0]
        this.GetICanPostData.Applicant_BranchName = data[1]
      }
    },
    // 金融機構中文名稱
    Applicant_BranchName: {
      get() {
        return this.GetICanPostData.Applicant_BranchName
      }
    },
    // 銀行帳號
    Applicant_Account: {
      get() {
        return this.GetICanPostData.Applicant_Account
      },
      set(value) {
        this.GetICanPostData.Applicant_Account = value
      }
    }
  },
  methods: {
