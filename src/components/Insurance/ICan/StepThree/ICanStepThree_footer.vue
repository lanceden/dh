  ...
  <div class="footer">
    <div class="footer-content">
      <nav class="navbar navbar-dark row">
        ...
        <div class="col-sm-8 footer-title footer-right" @click="GoNext()">填寫受益人資料</div>
      </nav>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    GoPrev() {
      this.$router.push(`/ican-Estimate`)
    },
    GoNext() {
      // 戶籍地址-輸入新的戶籍地址
      if ($('#txtNewAddress2').val() !== '') {
        this.GetICanPostData.address2 = $('#txtNewAddress2').val()
      }
      // 通訊地址-輸入新的通訊地址
      if ($('#txtNewAddress1').val() !== '') {
        this.GetICanPostData.address1 = $('#txtNewAddress1').val()
      }
