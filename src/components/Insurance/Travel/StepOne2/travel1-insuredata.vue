    ...
    <div class="bg-radius">
      ...
      <form class="form-bottom">
        ...
        <div class="form-group row">
          ...
          <label v-show="this.$store.state.Travel.TRAVELPOSTDATA.PolicyData.InsuredInfo[index].HasAuthRep" class="col-sm-12 col-form-label insure-label">親愛的客戶謝謝您申購旅平險，因法令規定您的申請文件需另檢附相關證明文件。很抱歉這次無法於本系統進行投保。造成您的不便我們深感抱歉，再次感謝您的惠顧。</label>
          <div class="border-bottom-line col-sm-12"></div>
          <div class="top col-sm-12">
...
export default {
  ...
  created() {
    if (this.$store.state.Travel.TRAVELPOSTDATA.PolicyData.TravelCountry !== null) {
      this.OnEnsure(this.$store.state.Travel.TRAVELPOSTDATA.PolicyData.InsuredInfo[this.index].HasAuthRep === true ? 'yes' : 'no')
    }
  },
  data() {
    return {
  ...
  computed: {
    ...
    Name: {
      get() {
        return this.$store.state.Travel.TRAVELPOSTDATA.PolicyData.InsuredInfo[this.index].PersonalData.Name
      },
      set(value) {
        this.$store.state.Travel.TRAVELPOSTDATA.PolicyData.InsuredInfo[this.index].PersonalData.Name = value
      }
    },
    DobYear: {
      get() {
    ...
    ProposerInfoId: {
      get() {
        return this.$store.state.Travel.TRAVELPOSTDATA.PolicyData.InsuredInfo[this.index].PersonalData.ID
      },
      set(value) {
        this.$store.state.Travel.TRAVELPOSTDATA.PolicyData.InsuredInfo[this.index].PersonalData.ID = value
      }
    }
  },
  methods: {
    ...
    OnEnsure(target) {
      ...
      this.$store.state.Travel.TRAVELPOSTDATA.PolicyData.InsuredInfo[this.index].HasAuthRep = target === 'yes'
      console.log(this.$store.state.Travel.TRAVELPOSTDATA.PolicyData.InsuredInfo[this.index].HasAuthRep)
    }
  }
}
