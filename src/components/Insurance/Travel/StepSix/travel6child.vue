...
export default {
  ...
  computed: {
    ...
    DobYear: {
      get() {
        if (this.$store.state.Travel.TRAVELPOSTDATA.PolicyData.InsuredInfo[this.index].BeneficiaryData[0].Dob === '') {
          return 2019
        }
        let result = moment(this.$store.state.Travel.TRAVELPOSTDATA.PolicyData.InsuredInfo[this.index].BeneficiaryData[0].Dob, 'YYYY-MM-DD').format('YYYY-MM-DD').split('-')[0]
        return parseInt(result)
      },
      set(value) {
        this.$store.state.Travel.TRAVELPOSTDATA.PolicyData.InsuredInfo[this.index].BeneficiaryData[0].Dob = `${value}-${this.DobMonth}-${this.DobDay}`
      }
    },
    // 身故受益人生日-月
    DobMonth: {
      get() {
        ...
        let result = moment(this.$store.state.Travel.TRAVELPOSTDATA.PolicyData.InsuredInfo[this.index].BeneficiaryData[0].Dob, 'YYYY-MM-DD').format('YYYY-MM-DD').split('-')[1]
        return parseInt(result)
      },
      set(value) {
        this.$store.state.Travel.TRAVELPOSTDATA.PolicyData.InsuredInfo[this.index].BeneficiaryData[0].Dob = `${this.DobYear}-${value}-${this.DobDay}`
      }
    },
    // 身故受益人生日-日
    DobDay: {
      get() {
        ...
        let result = moment(this.$store.state.Travel.TRAVELPOSTDATA.PolicyData.InsuredInfo[this.index].BeneficiaryData[0].Dob, 'YYYY-MM-DD').format('YYYY-MM-DD').split('-')[2]
        return parseInt(result)
      },
      set(value) {
        this.$store.state.Travel.TRAVELPOSTDATA.PolicyData.InsuredInfo[this.index].BeneficiaryData[0].Dob = `${this.DobYear}-${this.DobMonth}-${value}`
      }
    },
    // 身故受益人地址-縣市
    city3: {
      get() {
        return this.$store.state.Travel.TRAVELPOSTDATA.PolicyData.InsuredInfo[this.index].BeneficiaryData[0].Address.City || 0
      },
      set(value) {
        this.$store.state.Travel.TRAVELPOSTDATA.PolicyData.InsuredInfo[this.index].BeneficiaryData[0].Address.City = value
        // 重新選取縣市, 要更新區域下拉框並清空區域原先的值
        this.FuncGetDistrictData(value)
        this.$store.state.Travel.TRAVELPOSTDATA.PolicyData.InsuredInfo[this.index].BeneficiaryData[0].Address.District = 0
      }
    },
    // 身故受益人地址-區域
    district3: {
      get() {
        if (this.$store.state.Travel.TRAVELPOSTDATA.PolicyData.InsuredInfo[this.index].BeneficiaryData[0].Address.District === 0) return 0
        return (`${this.$store.state.Travel.TRAVELPOSTDATA.PolicyData.InsuredInfo[this.index].BeneficiaryData[0].Address.District}`) || 0
      },
      set(value) {
        // item.Zip|item.Area
        this.$store.state.Travel.TRAVELPOSTDATA.PolicyData.InsuredInfo[this.index].BeneficiaryData[0].Address.District = value
      }
    },
    // 身故受益人地址-路
    road3: {
      get() {
        return this.$store.state.Travel.TRAVELPOSTDATA.PolicyData.InsuredInfo[this.index].BeneficiaryData[0].Address.Street
      },
      set(value) {
        this.$store.state.Travel.TRAVELPOSTDATA.PolicyData.InsuredInfo[this.index].BeneficiaryData[0].Address.Street = value
      }
    }
  },
  methods: {
